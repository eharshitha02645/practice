...
public class ManagementRepository implements EmployeeDetailsCrudRepository {
	...
	public boolean save(PersonalDetails obj) {
	   ...
	   String sql="insert into test.employee_details values(?,?,?,?,?,?,?)";
	   int rowUpdated=0;
	   try {
	...
	public boolean updateEmailPhone(String firstName, String email, long phoneNumber) throws ElementNotFoundException {
        String sql="update test.employee_details set email=?,phone_number=? where first_name=?";
        int rowUpdated=0;
        try {
			PreparedStatement pstmt=con.prepareStatement(sql);
	...
	public boolean delete(String firstName) throws ElementNotFoundException {
        String sql="delete from test.employee_details where first_name=?";
        int rowDeleted=0;
		try {
			PreparedStatement pstmt=con.prepareStatement(sql);
	...
	public Collection<PersonalDetails> findAll() {
		...
		String sql="select * from test.employee_details";
		try {
			PreparedStatement pstmt=con.prepareStatement(sql);
			...
			while(rs.next()) {
			 ...
			 Date wedDate= rs.getDate("wedDate");
	...
	@Override
	public Collection<PersonalDetails> findByFnamePhone(String firstName, long phoneNumber) throws ElementNotFoundException {
		Collection<PersonalDetails> list=this.allRecords.stream().filter(e->e.getPhoneNumber()==phoneNumber&&e.getFirstName().equals(firstName)).collect(toList());
		return list;
	}

	@Override
	public Collection<String> findByDob(LocalDate dob) throws ElementNotFoundException {
		 Map<String, String> map=this.allRecords.stream().filter(e->e.getDateOfBirth().equals(dob))
	...
	public Collection<String> findByWed(LocalDate weddingDate) throws ElementNotFoundException {
		Map<String, Long> map=this.allRecords.stream().filter(e->e.getWeddingDate().equals(weddingDate))
	...
	public Collection<PersonalDetails> findByFirstName(String firstName) throws ElementNotFoundException {
		Collection<PersonalDetails> list=this.allRecords.stream().filter(e->e.getFirstName().contentEquals(firstName)).collect(toList());
